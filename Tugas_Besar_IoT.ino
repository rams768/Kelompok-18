...
String apiKey = "H6DH7IQ9RVB7MT7P";
...
static boolean serialVisual = true; // Set to 'false' by Default. Re-set to 'true' to see Arduino Serial Monitor Visual Pulse
...
void setup(){
  ...
  String cmd="AT+CWJAP=\"No\",\"1234567890\"";
  ser.println(cmd);
  delay(1000);
...
ISR(TIMER2_COMPA_vect){ //triggered when Timer2 counts to 124{
  ...
  if (N > 250){ // avoid high frequency noise
    if ( (Signal > thresh) && (Pulse == false) && (N > (IBI/5)*3)){
      ...
      BPM = 23500/runningTotal; // how many beats can fit into a minute? that's BPM!
